...
import { queryIndicationTpls, deleteIndicationTpl } from '@/services/indication';
...
export default class Template extends Component {
  ...
  getData = async () => {
    ...
    const tplsRes = await queryIndicationTpls(tplsParams);
  ...
  goToEdit = (tplId) => {
    history.push({
      pathname: '/basicData/indication/templateEdit',
      query: {
        tplId,
      },
    });
  ...
  disabledConfirm = async (tplId) => {
    const res = await deleteIndicationTpl({
      tplId,
    });
    if (res) {
      await this.getData();
