...
import { getHospitalDrugToAdd, addHospitalDrug } from '@/services/hospital-side';
...
export default class DrugChangeAddEdit extends Component {
  constructor(props) {
    super(props);
  ...
  listenLocation = async () => {
    const { saasDrugId } = this.props.location.query;
    if (saasDrugId) {
      const res = await getHospitalDrugToAdd({ saasDrugId });
      if (res) {
  ...
  onFinish = async (values) => {
    const res = await addHospitalDrug({ model: { ...values } });
    if (res) {
      ...
      this.formRef.current.resetFields();
      history.replace('/drugChange/management/management');
    }
  };
